...
SELECT e.empid, max(e.orderdate) AS maxorderdate
FROM Sales.Orders e
GROUP BY e.empid;
...
JOIN (	SELECT e.empid, max(e.orderdate) AS maxorderdate
		FROM Sales.Orders e
		GROUP BY e.empid) as o2
...
FROM MT o 
WHERE o.rownum between 11 and 20;
...
select j.empid, j.orderyear, j.qty,
		(select sum(j1.qty) 
		from dbo.jay j1
		where j.empid = j1.empid and j1.orderyear <= j.orderyear) as runqty
from dbo.jay j
